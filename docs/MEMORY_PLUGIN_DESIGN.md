...
| Step 2 | 升级 MemoryEntry：加 7 组维度字段（含 file_ref、lifecycle） | 写不同主体/类型/可见性/生命周期 |
| Step 3 | 读记忆按 visibility 鉴权 | 不同身份读，验证隔离 |
| Step 4 | 记忆互通查询（多条件过滤） | 跨 AI 历史、虚拟员工隔离 |
...
1. **权限体系**：`shared`/`confidential` 具体怎么定？（先等老师设计权限，再对齐）
2. **压缩定时**：30 天整、每日批处理？LLM 摘要的 prompt 谁来定？
3. **附件存储**：PoC 存本地目录，还是对象存储？附件大小上限？
4. **画像共享范围**：非敏感画像同步给哪些虚拟员工？（全部？同部门？）
5. **决策聚合**：读 AuditEvent 聚合，接口形态（单独查询还是并入 /memory）？
