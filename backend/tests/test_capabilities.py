...
def test_capability_catalog_unifies_plugins_and_skills(client):
    ...
    rows = resp.json()
    assert len(rows) == 16  # 12 plugins（含 L3 知识入口）+ 张三的 4 skills
...
def test_each_worker_gets_independent_harness_context(
    ...
    if approval_granted:
        # L3 执行必须先获得白名单；一次性任务审批不能绕过 P-DATA-003。
        grant = (
            db_session.query(models.EmployeePluginGrant)
            .filter_by(employee_id=employee_id, plugin_id=plugin_id, action="execute")
            .one()
        )
        grant.decision_mode = "allow"
