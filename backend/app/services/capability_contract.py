...
def _default_plugin_meta(plugin: models.Plugin) -> dict[str, Any]:
    internal_workflow = plugin.type == "workflow" and plugin.endpoint_ref.startswith("workflow://")
    primary = "workflow_engine" if internal_workflow else ("adapter" if plugin.type == "knowledge" else "harness")
    return {
        ...
        "executor": {
            ...
            "tool": "workflow_engine" if internal_workflow else "adapter",
...
def plugin_contract(plugin: models.Plugin) -> CapabilityContract:
    ...
    if primary not in {"adapter", "harness", "workflow_engine"}:
